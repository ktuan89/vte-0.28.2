...
_vte_terminal_emit_text_inserted(VteTerminal *terminal)
{
  ...
  fprintf(stderr, "emit text-inserted here\n");
...
{
  fprintf(stderr, "vte_terminal_new here\n");
...
{
  fprintf(stderr, "vte_terminal_emit_pending_text_signals\n");
	static struct {
		const char *name;
		GQuark quark;
	} non_visual_quarks[] = {
...
{
  fprintf(stderr, "vte_terminal_process_incoming\n");
	VteScreen *screen;
...
vte_terminal_get_text_include_trailing_spaces(VteTerminal *terminal,
					      ...
					      GArray *attributes)
{
  fprintf(stderr, "vte_terminal_get_text_include_trailing_spaces: %d\n", (int) terminal);
	g_return_val_if_fail(VTE_IS_TERMINAL(terminal), NULL);
	return vte_terminal_get_text_maybe_wrapped(terminal,
						   TRUE,
						   is_selected,
...
{
  fprintf(stderr, "vte_terminal_emit_pending_signals\n");
        GObject *object;
	GdkWindow *window;
